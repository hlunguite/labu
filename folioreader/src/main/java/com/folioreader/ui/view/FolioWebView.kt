...
import com.folioreader.util.TocUtils
...
//hlun remove highlight, copy share popup
  /*      viewTextSelection = LayoutInflater.from(ctw).inflate(R.layout.text_selection, null)
        ...
        viewTextSelection.copySelection.setOnClickListener {
            dismissPopupWindow()
          // ReadLocator lastRead = parentFragment.getLastReadLocator()
    ...
    fun onTextSelectionItemClicked(id: Int, selectedText: String?) {
        ...
        when (id) {
            ...
            R.id.shareSelection -> {
              //  parentFragment.mHtmlString
                Log.v(LOG_TAG, "-> onTextSelectionItemClicked -> shareSelection -> $selectedText")
               // Log.v(LOG_TAG," share click " + parentFragment.getHtml())
               // Log.v(LOG_TAG, "share click for " + TocUtils.get().getTitle(parentFragment.getCurrentId()))
    ...
    fun setHorizontalPageCount(horizontalPageCount: Int) {
        ...
        uiHandler.post {
            webViewPager = (parent as View).findViewById(R.id.webViewPager1)
    ...
    fun setSelectionRect(left: Int, top: Int, right: Int, bottom: Int) {
       ...
       /* hlun val currentSelectionRect = Rect()
