...
class FolioActivity : AppCompatActivity(), FolioActivityCallback, MediaControllerCallback,
    ...
    private var _curentLabu: Labu ? = null;
    companion object {
    ...
    private val currentFragment: FolioPageFragment?
        get() = if (mFolioPageFragmentAdapter != null && mFolioPageViewPager != null) {
            Log.d("HLUNBOI", "get current fragment" + mFolioPageViewPager!!.currentItem)
    ...
    override fun onNewIntent(intent: Intent) {
        ...
        setIntent(intent)
        Log.v(LOG_TAG, "-> onNewIntent folioreader")
    ...
    override fun onResume() {
        ...
        topActivity = true
        CurrentActivity.getInstance().currentActivity = this
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d("folioactivity", "calling")

        setConfig(savedInstanceState)
        initDistractionFreeMode(savedInstanceState)

        setContentView(R.layout.folio_fab_activity)
        ...
        //speedDialView!!.inflate(R.menu.speeddial)
    ...
    private fun initBook() {
        ...
        Log.d(LOG_TAG,"bookfilename " + bookFileName)
        ...
        Log.d(LOG_TAG, "port no " + portNumber)

        portNumber = AppUtil.getAvailablePortNumber(portNumber)
        Log.d(LOG_TAG, "port no after " + portNumber)
        ...
        Log.d(LOG_TAG, "streamerUrl " + streamerUrl )
    ...
    private fun onBookInitSuccess() {
        ...
        var size : Int = spine!!.size
        var currentlabusongs:Int = LabuData.get().currentLabu._noofSong
        Log.d("numberofsong", " is "  + size + " from current labu " + currentlabusongs);
       // Log.d("OnBookInitSucess", title.hashCode().toString())
        if (mBookId == null) {
        ...
        configFolio()
        //val publication = pubBox!!.publication
        TocUtils.get().populateTOC(publication)
    }

    override fun getStreamerUrl(): String {
       // Log.d(LOG_TAG, " getStreamerUrl " + streamerUrl)
        if (streamerUri == null) {
            streamerUri = Uri.parse(String.format(STREAMER_URL_TEMPLATE, LOCALHOST, portNumber, bookFileName))
            Log.d(LOG_TAG, " getStreamerUrl streamerUri " + streamerUri.toString())

        }
        //Log.d(LOG_TAG, " getStreamerUrl end " + streamerUrl)

        return streamerUri.toString()
    }

    override fun onDirectionChange(newDirection: Config.Direction) {
        Log.v(LOG_TAG, "-> onDirectionChangeBegin")
        ...
        Log.d(LOG_TAG,"onCreateView FolioPageFragmentAdapter ondirectionchange")
        ...
        Log.v(LOG_TAG, "-> onDirectionChangeDone")
     ...
     */
    override fun goToChapter(href: String, position : Int): Boolean {
        Log.d("GOTOCHAPTER", href + " position " + position + " size " + spine!!.size)
        if (position >= spine!!.size) {
            Log.d("GOTOCHAPTER", "index " + position + " out of range " + spine!!.size)
            return false;
        }
        if (position > -1) {
        ...
        if (href.isEmpty() == true ) return false;
	    var href1 =  href.replace("&","%26")
        href1 = href1.replace("'","%27")
        href1 = href1.replace(",","%2C")
        for (link in spine!!) {
            if (href1.contains(link.href!!)) {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == RequestCode.SEARCH.value) {
            ...
            if (resultCode == SearchActivity.ResultCode.ITEM_SELECTED.value) {
                var lano = data.getIntExtra(SearchActivity.BUNDLE_SEARCH_LANO,0)
                if (mFolioPageViewPager == null) return

                /* hlunguite searchLocator = data.getParcelableExtra(EXTRA_SEARCH_ITEM)
            ...
            if (type == CHAPTER_SELECTED) {
                goToChapter(data.getStringExtra(SELECTED_CHAPTER_POSITION), data.getIntExtra(SELECTED_CHAPTER_INDEX,-1))
    ...
    private fun configFolio() {
        ...
        Log.d(LOG_TAG, "onCreateView create new FolioPageFragmentAdapter configFolio")
        mFolioPageFragmentAdapter = FolioPageFragmentAdapter(
            supportFragmentManager,
            spine, bookFileName, mBookId
        )
        Log.d(LOG_TAG, "onCreateView create new FolioPageFragmentAdapter configFolio after")
        ...
        if (searchLocator != null) {
            Log.d(LOG_TAG, "search locator found ")
            ...
            Log.d(LOG_TAG, "search locator not found ")
            var songindex = intent.getIntExtra(FolioReader.SONG_NUMBER, -1)
            if (songindex  == -1) {
        ...
        Log.d(LOG_TAG, "onCreateView create end " + currentChapterIndex)
    }
    ...
    private fun clearSearchLocator() {
     ...
     fun initSpeedDial() {
         _curentLabu = LabuData.get().getCurrentLabu();
         //https://github.com/leinardi/FloatingActionButtonSpeedDial
         Log.d(LOG_TAG, "speeddial init")
      //  speedDialView = findViewById(R.id.speedDial) as SpeedDialView
        speedDialView = findViewById<SpeedDialView>(R.id.speedDial)
         speedDialView!!.addActionItem(SpeedDialActionItem.Builder(R.id.fab_bookmark, R.drawable.ic_bookmark)
                 .setTheme(R.style.AppTheme_speeddial).create())
         speedDialView!!.addActionItem(SpeedDialActionItem.Builder(R.id.fab_share, R.drawable.ic_share)
                 .setTheme(R.style.AppTheme_speeddial).create())
         ...
         speedDialView!!.setOnActionSelectedListener { speedDialActionItem ->
            when (speedDialActionItem.id) {
                  R.id.fab_music ->{
                      var keyid = _curentLabu!!.get_songKeyVal(currentFragment!!.getCurrentId());

                      //Log.d(LOG_TAG,"speeddial music selected")
                      var playnote = NotePlayer(this@FolioActivity, keyid)

                      false
                  }
                   R.id.fab_share->{
                      // Log.d(LOG_TAG, " speeddial html " +currentFragment!!.getHtml())
                      // Log.d(LOG_TAG, " speeddial html string " + Html.fromHtml(currentFragment!!.getHtml()))
                        currentFragment!!.shareLa()
                       //UiUtil.share(this@FolioActivity, currentFragment!!.getHtml())

                    //   Log.d(LOG_TAG,"speeddial share selected")
                       false
                   }
                   R.id.fab_bookmark ->{
                       var title = _curentLabu!!.get_laNoNMin(currentFragment!!.getCurrentId())
                        Log.d("speeddial", "bookmark click" + currentFragment!!.getCurrentId())
                       currentFragment!!.bookmark(title)
                    false
                   }
                   R.id.fab_solfa -> {
                       Log.d("speedial","solfa click")
         ...
         speedDialView!!.setOnChangeListener(object : SpeedDialView.OnChangeListener {
             ...
             override fun onMainActionSelected(): Boolean {
                 Log.d(LOG_TAG,"speeddial main action selected")
                 return false // true to keep the Speed Dial open
             }

             override fun onToggleChanged(isOpen: Boolean) {
                 if (isOpen ) {
                     Log.d("speeddial", "open")
                     var keyid = _curentLabu!!.get_songKeyVal(currentFragment!!.getCurrentId())
                     Log.d("speeddial", "keyid " + keyid)
                    if (keyid > 0) {
                        speedDialView!!.addActionItem(SpeedDialActionItem.Builder(R.id.fab_music, R.drawable.ic_music)
                                .setTheme(R.style.AppTheme_speeddial).create())
                    }

                     var pdfid = _curentLabu!!.get_songPdfPage(currentFragment!!.getCurrentId())
                     Log.d("speeddial", "pdfid " + pdfid)

                     if (pdfid > 0) {
                        speedDialView!!.addActionItem(SpeedDialActionItem.Builder(R.id.fab_solfa, R.drawable.ic_solfa)
                                .setTheme(R.style.AppTheme_speeddial).create())
                    }
                 }else {
                     Log.d("speeddial", "close")

                     speedDialView!!.removeActionItemById(R.id.fab_music)
                     speedDialView!!.removeActionItemById(R.id.fab_solfa)

                 }
             }
         })

     //mShare.setTheme(R.style.AppTheme_speeddial)
        //actionitemShare.create()
        //speedDialView!!.addActionItem(actionitemShare)
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent?): Boolean {
        ...
        {
            var str = ""
            goToChapter(str, currentChapterIndex - 1)
            return true;
        }
        if((keyCode == KeyEvent.KEYCODE_VOLUME_UP)){
            var size = spine!!.size
            if ((currentChapterIndex + 1) < size) {
                var str = ""
...
// need this to supress key press sound
            return true;
        }
